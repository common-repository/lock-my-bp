...
define( 'BPLOCK_PLUGIN_VERSION', '1.9.3' );
...
function bplock_required_plugin_admin_notice() {
	...
	/* translators: %1$s: BuddyPress Lock, %2$s: BuddyPress */
...
function bplock_activation_redirect_settings( $plugin ) {
	...
	if ( plugin_basename( __FILE__ ) === $plugin && class_exists( 'Buddypress' ) ) {
		if ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'activate' && isset( $_REQUEST['plugin'] ) && $_REQUEST['plugin'] == $plugin) { //phpcs:ignore
			wp_safe_redirect( admin_url( 'admin.php?page=bp-lock' ) );
			exit;
		}
	}
	if ( $plugin == $_REQUEST['plugin'] && class_exists( 'Buddypress' ) ) {
		if ( isset( $_REQUEST['action'] ) && $_REQUEST['action']  == 'activate-plugin' && isset( $_REQUEST['plugin'] ) && $_REQUEST['plugin'] == $plugin) { //phpcs:ignore		
			set_transient( '_bplock_is_new_install', true, 30 );
		}
	}
}
add_action( 'activated_plugin', 'bplock_activation_redirect_settings' );

function bplock_do_activation_redirect() {
	if ( get_transient( '_bplock_is_new_install' ) ) {
		delete_transient( '_bplock_is_new_install' );
		wp_safe_redirect( admin_url( 'admin.php?page=bp-lock' ) );
	}
}
add_action( 'admin_init', 'bplock_do_activation_redirect' );
