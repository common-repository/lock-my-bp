...
class Bp_Lock_Public
{
	...
	{
		wp_enqueue_style($this->plugin_name . '-font-awesome', BPLOCK_PLUGIN_URL . 'admin/css/font-awesome.min.css', array(), $this->version, 'all');
		wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/bp-lock-public.css', array(), $this->version, 'all');
	}
	...
	{
		wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/bp-lock-public.js', array('jquery'), $this->version, false);
		wp_localize_script(
			$this->plugin_name,
			'bplock_public_js_object',
			array(
				'ajaxurl'             => admin_url('admin-ajax.php'),
				'nonce'               => wp_create_nonce('bp_lock_nonce'),
				'is_recaptcha_active' => class_exists('Recaptcha_For_BuddyPress'),
			)
		);
	}
	 ...
	 * @since    1.0.0
	 *
	 * @param string $template BuddyPress Templates.
...
?>
				...
				if (file_exists($bplock->locked_content_template)) {
					$template = $bplock->locked_content_template;
				}
			}
		}
		return $template;
	...
	/**
	 * Exclude locked content from search results.
	 *
	 * @since 1.0.0
	...
	/**
	 * Modify the BuddyPress components template.
	 *
	 * @since    1.0.0
	 *
	 * @param array  $found_template Array of templates to attempt to load.
	 * @param string $templates Located template path.
	 * @return string The modified template.
	 */
...
<?php
	 ...
	 * @since 1.0.0
	 */
	public function bplock_login()
	{
		if (isset($_POST['nonce']) && !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'bp_lock_nonce')) {
			exit();
		}
		if (isset($_POST['action']) && 'bplock_login' === $_POST['action']) {
	 ...
	 * @since 1.0.0
	 */
	public function bplock_register()
	{
		if (isset($_POST['nonce']) && !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'bp_lock_nonce')) {
			exit();
		}
		if (isset($_POST['action']) && 'bplock_register' === $_POST['action']) {
	 ...
	 * @param string $default_path Default path to template files.
	 */
	public function bplock_get_template($template_name, $args = array(), $template_path = '', $default_path = '')
	{
		if (is_array($args) && isset($args)) {
			extract($args);
		}

		$template_file = $this->bplock_locate_template($template_name, $template_path, $default_path);
